...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"saga": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + saga.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + saga.blockNumber + " (" + new Date(1000 * saga.getBlock(saga.blockNumber).timestamp) + ")\n";
		try {
